-- ============================================================
-- COMBINED MIGRATIONS (001 + 002 + 003 + 004)
...
CREATE TYPE integration_provider AS ENUM (
  ...
  'microsoft_calendar', 'onedrive', 'slack', 'notion',
  'telegram', 'whatsapp', 'todoist', 'linear', 'hubspot', 'salesforce'
);

CREATE TYPE integration_status AS ENUM ('connected', 'disconnected', 'error', 'pending');

CREATE TYPE briefing_item_type AS ENUM (
  'email', 'calendar_event', 'commitment', 'relationship_alert',
  'document', 'task', 'slack_message'
);

CREATE TYPE briefing_item_section AS ENUM (
  'priority_inbox', 'commitment_queue', 'at_risk', 'todays_schedule',
  'decision_queue', 'quick_wins', 'people_context'
);

CREATE TYPE commitment_status AS ENUM ('open', 'resolved', 'snoozed', 'dismissed', 'delegated');
CREATE TYPE commitment_confidence AS ENUM ('high', 'medium', 'low');

CREATE TYPE pending_action_type AS ENUM (
  'send_email', 'send_message', 'create_task',
  'reschedule_meeting', 'create_calendar_event', 'update_notion_page'
);
...
CREATE TYPE message_delivery_channel AS ENUM ('telegram', 'whatsapp', 'in_app', 'sms');
CREATE TYPE heartbeat_frequency AS ENUM ('realtime', 'hourly', 'daily');
CREATE TYPE data_region AS ENUM ('me-south-1', 'eu-central-1', 'us-east-1');
CREATE TYPE subscription_tier AS ENUM ('free', 'pro', 'power', 'team');
...
CREATE POLICY "Users can manage own onboarding data" ON public.onboarding_data FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own audit log" ON public.integration_audit_log FOR SELECT USING (auth.uid() = user_id);
...
CREATE TABLE public.briefings (
  ...
  meeting_preps     JSONB DEFAULT '[]'::jsonb,
...
CREATE POLICY "Users can view own briefing items" ON public.briefing_items FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can update own briefing items" ON public.briefing_items FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own interactions" ON public.contact_interactions FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Users can manage own pending actions" ON public.pending_actions FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users can manage own heartbeat config" ON public.heartbeat_config FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own heartbeat runs" ON public.heartbeat_runs FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Users can manage own inbox items" ON public.inbox_items FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own telegram sessions" ON public.telegram_sessions FOR SELECT USING (auth.uid() = user_id);
...
CREATE TRIGGER update_integrations_updated_at
  BEFORE UPDATE ON public.user_integrations FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_commitments_updated_at
  BEFORE UPDATE ON public.commitments FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_inbox_items_updated_at
  BEFORE UPDATE ON public.inbox_items FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_heartbeat_config_updated_at
  BEFORE UPDATE ON public.heartbeat_config FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- ============================================================
-- 002: VECTOR SEARCH FUNCTION
-- ============================================================
...
RETURNS TABLE (
  id uuid, provider text, source_id text,
  content_summary text, metadata jsonb, similarity float
)
LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  RETURN QUERY
  SELECT dc.id, dc.provider, dc.source_id, dc.content_summary, dc.metadata,
...
-- ============================================================
-- 004: BETA LAUNCH (waitlist, feedback, admin)
-- Note: 003 (meeting_preps) is already included in briefings table above
-- ============================================================
