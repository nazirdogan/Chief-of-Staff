...
export const AskSceneV2: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const { start, duration } = SCENES.ask;
  ...
  // Show truncated response (fast reveal)
  ...
  const revealChars = Math.floor(shortResponse.length * interpolate(rel, [48, 120], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }));
